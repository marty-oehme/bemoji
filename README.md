...
but is not an issue with `wl-ime-type` if the application supports it.
...
`wl-ime-type` only works with applications which support the Wayland `text-input-v3` protocol.
Most Wayland applications do, including GTK3/4 and Qt5/6 applications.  
